...
function poi_time_since($older_date, $comment_date = false, $text = false)
{
  $chunks = array(
    ...
    /*天前*/
    array(60 * 60, __(' hours ago', 'sakura')),
    /*小时前*/
    array(60, __(' minutes ago', 'sakura')),
    /*分钟前*/
...
if (version_compare($GLOBALS['wp_version'], '4.4-alpha', '<')) {
  wp_die(__('Please upgrade wordpress to version 4.4+', 'sakura'));
} /*请升级到4.4以上版本*/
// 提示
          ...
          <div class="main shadow">
            ...
            <div class="commentinfo">
              <section class="commeta">
                <div class="left">
                  <h4 class="author"><a href="<?php comment_author_url(); ?>"><?php echo get_avatar($comment->comment_author_email, '80', '', get_comment_author()); ?><?php comment_author(); ?> <span
                        class="isauthor" title="<?php esc_attr_e('Author', 'sakura'); ?>"></span></a></h4>
                </div>
                <div class="right">
  ...
  <?php if (current_user_can('level_10')) { ?>
    <div class="admin-login-check">
      ...
      <?php if (akina_option('login_urlskip')) { ?>
        <script>window.open("<?php bloginfo('url'); ?>/wp-admin/", 1); gopage("<?php bloginfo('url'); ?>", 0);</script>
      <?php } ?>
    </div>
  <?php } else { ?>
    <div class="user-login-check">
      <?php echo login_ok(); ?>
      <?php if (akina_option('login_urlskip')) { ?>
        <script>gopage("<?php bloginfo('url'); ?>", 0);</script>
      <?php } ?>
    </div>
  <?php
  }
...
function login_ok()
{
  ...
  wp_get_current_user();
  ?>
  <p class="ex-login-avatar"><a href="http://cn.gravatar.com/" title="<?php _e('Change avatar', 'sakura') /*更换头像*/?>"
      target="_blank" rel="nofollow"><?php echo get_avatar($current_user->user_email, '110'); ?></a></p>
  <p class="ex-login-username">
    <?php _e('Hello, ', 'sakura') /*你好，*/?><strong>
  ...
  <p class="ex-logout">
    ...
    <?php } ?>
    <a href="<?php echo wp_logout_url(get_bloginfo('url')); ?>" title="<?php _e('Logout', 'sakura') /*登出*/?>"
      target="_top"><?php _e('Sign out? ', 'sakura') /*登出？*/?></a>
  </p>
<?php
}
...
function the_headPattern()
{
  ...
  if (!is_home() && $full_image_url): ?>
    ...
    <div class="pattern-center <?php if (is_single()) {
      ...
      <div class="pattern-attachment-img lazyload"
        style="background-image: url(https://cdn.jsdelivr.net/gh/moezx/cdn@3.0.1/img/svg/loader/orange.progress-bar-stripe-loader.svg)"
        data-src="<?php echo $full_image_url; ?>"> </div>
  ...
  <?php elseif (is_single() && !($full_image_url)): ?>
    ...
    <div class="pattern-center <?php echo $center; ?>">
      <div class="pattern-attachment-img lazyload"
        style="background-image: url(https://cdn.jsdelivr.net/gh/moezx/cdn@3.0.1/img/svg/loader/orange.progress-bar-stripe-loader.svg)"
        data-src="<?php echo get_random_bg_url(); ?>"> </div>
  ...
  if (!is_home() && $full_image_url): ?>
    ...
    <div class="pattern-center <?php if (is_single()) {
      ...
      <div class="pattern-attachment-img" style="height: auto;">
        <video loop id="coverVideo" class='hls' style="width: 100%; height: 100%" <?php echo $video_poster_attr; ?>
...
function header_user_menu()
{
  ...
  if (is_user_logged_in()) {
    ...
    <div class="header-user-avatar">
      <img class="faa-spin animated-hover" src="<?php echo get_avatar_url($current_user->ID, 64); /*$ava;*/?>" width="30"
        height="30">
      <div class="header-user-menu">
        <div class="herder-user-name">Signed in as
          <div class="herder-user-name-u">
            <?php echo $current_user->display_name; ?>
          </div>
        </div>
    ...
    <div class="header-user-avatar">
      ...
      <div class="header-user-menu">
        <div class="herder-user-name no-logged">Whether to <a href="<?php echo $login_url; ?>" target="_blank"
...
function siren_private_message_hook($comment_content, $comment)
{
  ...
  if ($current_commenter['comment_author_email'] == $email || $parent_email == $current_commenter['comment_author_email'] || current_user_can('delete_user'))
    return $comment_content;
  if ($is_private)
    return '<i class="fa fa-lock" aria-hidden="true"></i> ' . __("The comment is private", "sakura") /*该评论为私密评论*/;
  return $comment_content;
}
